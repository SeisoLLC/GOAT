#!/usr/bin/env python3
"""
salacious-code-reviews constants
"""
...
PROMPT = (
    ...
    "the code. Only review lines of code which have been changed (added or removed) "
    "in the pull request. The code looks similar to the output of a git diff command. "
    "Lines which have been removed are prefixed with a minus (-) and lines which have "
    "been added are prefixed with a plus (+). Other lines are added to provide context "
    "but should be ignored in the review. Your review should evaluate the changed code "
    "using a risk score similar to a LOGAF score but measured from 1 to 5, where 1 is "
    "the lowest risk to the code base if the code is merged and 5 is the highest risk "
    ...
    "code cleaner, and maximizing the performance of the programming language. Flag "
